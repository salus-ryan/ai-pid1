...
mkdir -p "$OUT"
for f in rootfs.cpio.gz ai-pid1-usb.tar.gz ai-pid1-usb.iso ai-cortex-usb.tar.gz ai-cortex-usb.img mvp-check.log boot-smoke.log eval-results.json; do [ -f "$ROOT/$f" ] && cp -f "$ROOT/$f" "$OUT/"; done
...
Files:
  ...
  ai-cortex-usb.tar.gz  portable AI-native OS USB tree
  ai-cortex-usb.img     portable UEFI FAT image if generated
